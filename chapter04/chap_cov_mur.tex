...
Let $\mathcal{H}$ be a (finite dimensional) Hilbert space, $\saops{\mathcal{H}}$ the self-adjoint operators on $\mathcal{H}$, $\posops{\mathcal{H}}$ the cone of positive operators within $\saops{H}$, and $\Omega$ a (finite) set.  A map $\ope:\Omega\to \posops{\mathcal{H}}$ is an \emph{observable} if it is normalised
\begin{align}
  \sum_{\omega\in\Omega} \ope(\omega) &= \opi,
\end{align}
where $\opi$ is the identity operator on $\mathcal{H}$. We call $\Omega$ the outcome set of $\ope$.

A set of $n$ observables on the Hilbert space $\mathcal{H}$, $\sbuild{\ope_i\,}{\,i=1\hdots n}$, with outcome sets $\Omega_i$ is \emph{compatible} if there exists a joint observable, $\opj: \Omega \to \posops{\mathcal{H}}$,
\begin{align}
...
Given a pair of probability distributions, $S$ and $T$  over the same (finite) set $\Omega$, we define the following family of measures of how much they differ
% \begin{align}
%   \delta_p(S,T) &\defeq \pnorm{S(\omega) - T(\omega)}\\
...
% \end{align}
note that $\delta_p(S,T) \geq 0$ with equality if and only if $S = T$ and
\begin{align}
  \delta_p(S,T) \leq 2^{\frac{1}{p}}, \quad \forall p\in [1,\infty].
\end{align}

Given an observable $\ope:\Omega\to\saops{\mathcal{H}}$ and a quantum state $\rho\in\saops{\mathcal{H}}$ on the same Hilbert space we can define a probability distribution over $\Omega$
\begin{equation}
  \ope^\rho(\omega) =   \tr{\ope(\omega)\rho}.
\end{equation}

We can then lift our distance measure on probability distributions with outcome set $\Omega$ to one on observables on the same set, simply by taking the $\sup$ of the distance for the probability distributions over all states
\begin{align}
  d_p(\ope, \opf) &\defeq \sup_{\rho\in\dops{\mathcal{H}}} \delta_p(\ope^\rho, \opf^\rho)\\
                  &= \sup_{\rho\in\dops{\mathcal{H}}} \left(\sum_{\omega\in\Omega}\abs{\ope^\rho(\omega) - \opf^\rho(\omega)}^p\right)^{\frac{1}{p}},
\end{align}
where $\dops{\mathcal{H}}$ is the set of trace $1$ density operators within $\posops{\mathcal{H}}$. Note that $\dops{\mathcal{H}}$ is a compact set, so the $\sup$ is actually achieved by some $\rho^*$. We specialise the definition \eqref{eqn:general-ur-definition} of the uncertainty region to these error measures, where $\ope_i:\Omega_i\to\posops{\mathcal{H}}$ are the target observables
\begin{equation}
  S_p(\ope_1, \ope_2\hdots \ope_n) = \sbuild{(d_p(\ope_1, \opf_1), d_p(\ope_2, \opf_2)\hdots d_p(\ope_n, \opf_n))}{\opf_i:\Omega_i\to\posops{\mathcal{H}} \text{ are compatible}},\quad p\in [1,\infty].
\end{equation}

It is useful to embed the set of observables with a fixed outcome set $\Omega$ in the real vector space of all maps $\Omega\to \saops{\mathcal{H}}$.
%\begin{align}
%  \left( a \ope + b \opf\right)(\omega) = a\ope(\omega) + b\opf(\omega).
%\end{align}
This space can be equipped with the ``$\sup$-$p$'' norm
\begin{equation}
  \pnorm{\ope} =  \sup_{\rho\in\saops{\mathcal{H}}} \pnorm{\ope^\rho},
\end{equation}
allowing us to rewrite our distance
\begin{align}
...
\begin{lem}[Increasing the error - $\infty$-norm]
  ...
  \begin{proof}
    Let $\Omega = \prod_i \Omega_i$ be the Cartesian product of the outcome sets, since $\bov\in S_\infty(\ope_1\hdots\ope_n)$ there exists a compatible family of observabless $\opf_i$ with joint $\opj:\Omega\to\saops{\mathcal{H}}$ such that
    \begin{align}
      d_\infty(\ope_i, \opf_i) = v_i
    \end{align}
    Now define
    \begin{align}
      \tilde{\opj}&:\Omega\to \saops{\mathcal{H}}\\
    ...
    \begin{align}
      d_\infty(\ope_i, \opf_{i\lambda}) &= \sup_{\rho\in\dops{\mathcal{H}}}\max_{\omega\in\Omega} \abs{\tr{\rho(\ope_i(\omega) - \opf_{i\lambda}(\omega))}}\\
                                         &= \sup_{\rho\in\dops{\mathcal{H}}}\max_{\omega\in\Omega} \abs{(1-\lambda)\tr{\rho(\ope_i(\omega) - \opf_{i})} + \lambda\tr{\rho(\ope_i(\omega) - \tilde{\opf}_{i})}(\omega)}g,
    \end{align}
...
\begin{lem}[Increasing the error - $p$-norm]
  ...
  Let $\{\ope_i\}$ be a family of observables with outcome sets $\Omega_i$. Choose $i\in 1\hdots n$ and $\bov = (v_1\hdots v_i\hdots v_n) \in S_p(\ope_1\hdots\ope_n)$, such that there exists some $\omega^*\in\Omega_i$ where $\tr{\ope_i(\omega^*)\rho^*} = 1$ for some $\rho^*\in\dops{\mathcal{H}}$ then $v_i \leq v_i^\prime \leq 2^{\frac{1}{p}} \implies \bov^\prime = (v_1\hdots v_i^\prime\hdots v_n) \in S_p(\ope_1\hdots\ope_n)$.
  \begin{proof}
    Let $\Omega = \prod_i \Omega_i$ be the Cartesian product of the outcome sets, since $\bov\in S_p(\ope_1\hdots\ope_n)$ there exists a compatible family of observables $\opf_i$ with joint $\opj:\Omega\to\saops{\mathcal{H}}$ such that
    \begin{align}
      d_p(\ope_i, \opf_i) = v_i
    \end{align}
...
Given a group $G$, with an action $f_g:\Omega\to\Omega$, $g\in G$, and a representation $\{R_g\, | \, g\in G\}$ as positive linear maps acting on $\saops{\mathcal{H}}$ we say an observable $E:\Omega\to\saops{\mathcal{H}}$ is \emph{covariant} if
\begin{equation}
...
\begin{align}
  \opi &= \sum_\omega \ope(f_g(\omega))\\
       ...
       &= R_g\left[ \opi\right],
\end{align}
Given $G$, $f_g$, and $R_g$ we can define the \emph{invariant mean}, which maps POVMs to POVMs
\begin{equation}
  M_{R,f}(\ope)(\omega) = \frac{1}{\abs{G}}\sum_{g\in G} R_{g^{-1}}\! \left[\ope(f_g(\omega))\right]
\end{equation}
This has some useful properties
\begin{lem}
  \label{lem:cov-obs-invariant}
  Covariant observables are unchanged by the invariant mean.
\end{lem}
\begin{lem}
  For any observable $\ope:\Omega\to\saops{\mathcal{H}}$, with $R$ and $f$ defined as above, the invariant mean $M_{R,f}(\ope)$ is a covariant observable
\end{lem}
\begin{lem}
  \label{lem:cov-reduces-p-norm}
  For any group $G$, with $R_g$ and $f_g$ defined as above, $M_{R,f}$ acts to reduce the $\sup$-$p$-norm of maps from $\Omega$ to $\saops{\mathcal{H}}$.
  \begin{proof}
    First define the observables
    \begin{align}
      \ope_g:\Omega&\to\posops{\mathcal{H}}\\
...
\begin{lem}\label{lem:margins-inv-mean-same}
  For $i\in 1\hdots n$ let $\ope_i:\Omega_i \to \posops{\mathcal{H}}$ be a compatible family of observables, and $\sbuild{G_i}{i\in 1\hdots n}$ be a set of groups, such that $G_i$ has action $f_g^i$ on $\Omega_i$. Let $\Omega = \prod_i \Omega_i$ be the Cartesian product, $G = \prod_i G_i$ the direct product, and $\pi$, $\mu^i$ the product and marginal actions of $G$ respectively. Let $\sbuild{R_g}{g\in G}$ be a representation of $G$ as positive, unital, linear maps acting on $\saops{\mathcal{H}}$. If $\opj$ is any Cartesian joint observable for the $\ope_i$, and $\tilde{\opj}_i$ the $i^\text{th}$ margin of $M_{R,\pi}(\opj)$,
  \begin{align}
    \tilde{\opj}_i&: \Omega_i \to \posops{\mathcal{H}}\\
...
\begin{thm}
  ...
  Let $\sbuild{\ope_i}{ i\in 1\hdots n}$ be a family of (not necessarily compatible) observables, $\ope_i:\Omega_i \to \posops{\mathcal{H}}$, and $\sbuild{G_i}{i\in 1\hdots n}$ be a set of groups, such that $G_i$ has action $f_g^i$ on $\Omega_i$. Let $\Omega = \prod_i \Omega_i$ be the Cartesian product, $G = \prod_i G_i$ the direct product, and $\pi$, $\mu^i$ the product and marginal actions of $G$ respectively. Let $\sbuild{R_g}{g\in G}$ be a representation of $G$ as positive, unital, linear maps acting on $\saops{\mathcal{H}}$ such that
  \begin{align}
    M_{R,\mu^i}(\ope_i) = \ope_i.
  \end{align}
  Then for any compatible family of observables $\sbuild{\opf_i}{ i\in 1\hdots n}$, $\opf_i:\Omega_i \to \posops{\mathcal{H}}$, with joint observable $\opj:\Omega\to\posops{\mathcal{H}}$, the observables
  \begin{align}
    \tilde{\opf}_i &= M_{R, \mu^i}(\opf_i)
  \end{align}
  are compatible, with joint $\tilde{\opj} = M_{R, \pi}(\opj)$, and $\forall p\in [1,\infty]$
  \begin{align}
    \label{eqn:inv-mean-reduce-error}
    d_p(\tilde{\opf}_i, \ope_i) \leq d_p(\opf_i, \ope_i).
  \end{align}
  \begin{proof}
    ...
    \begin{align}
      d_p(\tilde{\opf}_i, \ope_i) &= \sup_\rho \pnorm{M_{R, \mu^i}(\opf) - \ope_i}\\
      \label{eqn:line-target-inv}
                                  &= \sup_\rho \pnorm{M_{R, \mu^i}(\opf) - M_{R, \mu^i}(\ope_i)}\\
                                  &= \sup_\rho \pnorm{M_{R, \mu^i}(\opf - \ope_i)}\\
                                  &\leq \sup_\rho \pnorm{\opf - \ope_i}\\
      \label{eqn:line-inv-mean-contraction}
                                  &= d_p(\opf_i, \ope_i).
    \end{align}
    Note that \eqref{eqn:line-target-inv} is a consequence of assuming the target observables are unchanged by the invariant mean, and \eqref{eqn:line-inv-mean-contraction} is the result of lemma \ref{lem:cov-reduces-p-norm}.
  \end{proof}
\end{thm}
...
Let $\mathcal{H}$ be a Hilbert space of dimension $n\in\mathbb{N}$, $n \geq 2$, $\sbuild{\ket{g}}{g \in \cyc[n]}$ be an orthonormal set of vectors, hereafter called the \emph{computational basis} and let
\begin{align}
...
\end{align}
be the well known \emph{Fourier basis}. It is easily verified that the $\ket{f_h}$ are an orthonormal basis for $\mathcal{H}$ and are \emph{mutually unbiased} with the computational basis. We define sharp observables for these bases
\begin{align}
  \opa&:\cyc[n] \to \posops{\mathcal{H}} & \opb&:\cyc[n] \to \posops{\mathcal{H}}\\
...
\begin{align}
  U_k V_q \rho\, V_q^\dagger U^\dagger_k = V_q U_k \rho\,  U^\dagger_k V_q^\dagger, \quad \forall \rho\in\saops{\mathcal{H}}.
\end{align}
We therefore consider the linear maps
\begin{align}
  R_{k,q}&:\saops{\mathcal{H}}\to \saops{\mathcal{H}}\\
...
There is a one-to-one relation between covariant joint observables $\opj:\cyc[n]\times\cyc[n]\to\posops{\mathcal{H}}$ and trace one positive operators on $\mathcal{H}$ given by 
\begin{align}
...
\begin{align}
  \opc&:\cyc[n] \to \posops{\mathcal{H}} & \opd&:\cyc[n] \to \posops{\mathcal{H}}\\
...
\begin{align}
  \mathcal{M}&: \saops{\mathcal{H}} \to M_2(\mathbb{C})\\
